...
LONG EmacPsDmaSingleFrameIntrExample(XEmacPs *EmacPsInstancePtr)
{
    ...
    /* ========================================================== */
    /* 1. Prepare thousands of packets in DDR memory              */
    /* ========================================================== */
    ...
    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
        ...
        /*
         * PROOF OF DDR READ:
         * Inject the packet index (i) into the first bytes of the payload.
         * You will see this specific byte changing across packets in Wireshark.
         */
        DdrTxFrames[i][14] = (char)(i & 0xFF);         // Low byte of counter
        DdrTxFrames[i][15] = (char)((i >> 8) & 0xFF);  // High byte of counter
    }

    /* Flush DDR array from cache to physical memory */
    ...
    /* Clear and Flush Rx Buffer (Required by driver structure) */
    ...
    if (Status == XST_SUCCESS) {
        ...
        XEmacPs_BdRingToHw(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), 1, BdRxPtr);
        if (GemVersion > 2 && EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
            Xil_DCacheFlushRange((UINTPTR)BdRxPtr, 64);
        }
    }

    /* Allocate TEST_PACKETS_COUNT Tx BDs for our DDR array */
    ...
    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
            ...
            /*
                 * In the Xilinx GEM architecture, since each descriptor contains an independent frame,
                 * the Last (EOF) bit must be set for every single descriptor.
            */
    ...
    /* ========================================================== */
    /* 4. Start Burst Transmission & Measure Throughput           */
    /* ========================================================== */
    XTime tStart, tEnd;
    FramesTx = 0; /* Reset global interrupt counter */
    ...
    /*
     * Wait for interrupt. DMA triggers ONE interrupt after the entire
     * burst is completed or disabled by the driver.
     */
    while (FramesTx == 0);

    XTime_GetTime(&tEnd);

    /* Integer calculations to prevent FPU linkage errors */
    ...
    xil_printf("Total Data: %lu Bytes\r\n", total_bytes);
    xil_printf("Time taken: %lu microseconds\r\n", (u32)time_taken_us);
    xil_printf("Throughput: %lu MB/s\r\n", throughput_MBps);
