...
LONG EmacPsDmaSingleFrameIntrExample(XEmacPs *EmacPsInstancePtr)
{
    ...
    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
        ...
        EmacPsUtilFrameSetPayloadData(&DdrTxFrames[i], PayloadSize);

        /* Inject the packet index into the payload for debugging */
        DdrTxFrames[i][14] = (char)(i & 0xFF);
        DdrTxFrames[i][15] = (char)((i >> 8) & 0xFF);
    }

    /* Flush TX DDR array from cache to physical memory */
    ...
    if (DetectedPhyAddr < 32) {
        xil_printf("Found PHY at address %lu\r\n", DetectedPhyAddr);
        do {
            XEmacPs_PhyRead(EmacPsInstancePtr, DetectedPhyAddr, 1, &PhyStatus);
            EmacpsDelay(1);
            xil_printf("Polling PHY status... (Reg 1 = 0x%04x)\r\n", PhyStatus);
        } while ((PhyStatus & 0x0004) == 0);

        xil_printf("Link is UP! Waiting 3 seconds for Windows network adapter to initialize...\r\n");
        EmacpsDelay(3);
        xil_printf("Starting MAC and Transmitting %d packets from DDR...\r\n", TEST_PACKETS_COUNT);
    } else {
    ...
    /* ========================================================== */
    /* 4 & 5. Start Simultaneous TX and RX                        */
    /* ========================================================== */
    XTime tStart, tEnd;
    FramesTx = 0;
    ...
    /* Disable receiving broadcast packets to block unwanted Windows traffic */
    XEmacPs_ClearOptions(EmacPsInstancePtr, XEMACPS_BROADCAST_OPTION);

    /* Start the device and initiate transmission */
    XEmacPs_Start(EmacPsInstancePtr);
    XEmacPs_Transmit(EmacPsInstancePtr);

    xil_printf("\r\n--- TX Started! Now monitoring BOTH TX and RX simultaneously... ---\r\n");

    int last_rx = 0;

    /*
     * The processor now waits in a single loop until BOTH operations finish.
     * Condition: Keep looping if we haven't received all packets OR haven't finished transmitting.
     * Note: FramesTx is incremented in the TX interrupt handler.
     */
    while ((FramesRx < TEST_PACKETS_COUNT) || (FramesTx == 0)) {

        /* Check if new packets have been received in the background */
        if (FramesRx != last_rx) {
            last_rx = FramesRx;
            xil_printf("\r\nReceived %d packets so far...\r\n", last_rx);

            int pkt_index = last_rx - 1;
            ...
            /* Hex Dump: Print the first 32 bytes of the sampled packet */
            xil_printf("--- Hex Dump of Packet [%d] (First 32 Bytes) ---\r\n", pkt_index);
    ...
    /* Calculate overall time and throughput */
    u64 time_taken_us = (1000000ULL * (tEnd - tStart)) / COUNTS_PER_SECOND;
    u32 total_bytes = TEST_PACKETS_COUNT * TxFrameLength;
    u32 throughput_MBps = 0;

    if (time_taken_us > 0) {
        throughput_MBps = (u32)((1000000ULL * total_bytes) / (1024ULL * 1024ULL * time_taken_us));
    }

    xil_printf("\r\n--- TX and RX Completed Successfully! ---\r\n");
    xil_printf("Total TX Data: %lu Bytes\r\n", total_bytes);
    xil_printf("Time taken for combined operation: %lu microseconds\r\n", (u32)time_taken_us);
    xil_printf("Estimated Throughput: %lu MB/s\r\n", throughput_MBps);

    /* Reclaim hardware TX BDs */
    if (XEmacPs_BdRingFromHwTx(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, &BdPtr) == 0) {
        EmacPsUtilErrorTrap("TxBDs were not ready for post processing");
        return XST_FAILURE;
    }

    /* Free the TX BDs */
    Status = XEmacPs_BdRingFree(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, BdPtr);
    if (Status != XST_SUCCESS) {
