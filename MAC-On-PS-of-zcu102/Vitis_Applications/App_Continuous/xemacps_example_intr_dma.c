...
#define TEST_PACKETS_COUNT 1000  // send 1000 packet one after another

#ifdef __ICCARM__
#pragma data_alignment = 64
EthernetFrame DdrRxFrames[TEST_PACKETS_COUNT];
#else
EthernetFrame DdrRxFrames[TEST_PACKETS_COUNT] __attribute__ ((aligned(64)));
#endif

#ifdef __ICCARM__
#pragma data_alignment = 64
EthernetFrame DdrTxFrames[TEST_PACKETS_COUNT];
#else
EthernetFrame DdrTxFrames[TEST_PACKETS_COUNT] __attribute__ ((aligned(64)));
#endif

#define RXBD_CNT TEST_PACKETS_COUNT
#define TXBD_CNT  TEST_PACKETS_COUNT
...
/**
*
* This function demonstrates the usage of the EMACPS by continuously sending and
* receiving frames in DMA interrupt mode.
* It generates a 20% duty cycle step signal, pushes it to DDR, and then to PC.
*
* @param	EmacPsInstancePtr is a pointer to the instance of the EmacPs
*		driver.
*
* @return	XST_SUCCESS to indicate success, otherwise XST_FAILURE.
*
* @note		This function enters an infinite loop for continuous processing.
*
*****************************************************************************/
LONG EmacPsDmaSingleFrameIntrExample(XEmacPs *EmacPsInstancePtr)
{
    ...
    /* ========================================================== */
    /* 1. Prepare packets in DDR memory                           */
    /* ========================================================== */
    char BroadcastMAC[] = { 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF };

    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
        EmacPsUtilFrameHdrFormatMAC(&DdrTxFrames[i], BroadcastMAC);
        EmacPsUtilFrameHdrFormatType(&DdrTxFrames[i], PayloadSize);

        /* Inject the 20% Duty Cycle step signal into the payload */
        GenerateDutyCycleSignal(&DdrTxFrames[i], PayloadSize);
    }

    /* Flush TX DDR array from CPU cache to physical memory (DDR) */
    if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
        Xil_DCacheFlushRange((UINTPTR)&DdrTxFrames, sizeof(EthernetFrame) * TEST_PACKETS_COUNT);
    }

    /* Clear and Flush RX Buffers */
    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
        EmacPsUtilFrameMemClear(&DdrRxFrames[i]);
    }
    if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
        Xil_DCacheFlushRange((UINTPTR)&DdrRxFrames, sizeof(EthernetFrame) * TEST_PACKETS_COUNT);
    }

    /* ========================================================== */
    /* 2. Setup DMA Descriptors (BDs) for RX and TX               */
    /* ========================================================== */

    /* Allocate and setup RX BDs */
    Status = XEmacPs_BdRingAlloc(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, &BdRxPtr);
    if (Status == XST_SUCCESS) {
        XEmacPs_Bd *CurRxBdPtr = BdRxPtr;
        for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
            XEmacPs_BdSetAddressRx(CurRxBdPtr, (UINTPTR)&DdrRxFrames[i]);
            CurRxBdPtr = XEmacPs_BdRingNext(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), CurRxBdPtr);
        }
        XEmacPs_BdRingToHw(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, BdRxPtr);

        if (GemVersion > 2 && EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
            Xil_DCacheFlushRange((UINTPTR)BdRxPtr, 64 * TEST_PACKETS_COUNT);
        }
    } else {
        EmacPsUtilErrorTrap("Error allocating RxBDs");
        return XST_FAILURE;
    }

    /* Allocate and setup initial TX BDs */
    Status = XEmacPs_BdRingAlloc(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, &BdPtr);
    if (Status != XST_SUCCESS) {
        EmacPsUtilErrorTrap("Error allocating TxBDs");
        return XST_FAILURE;
    }

    XEmacPs_Bd *CurBdPtr = BdPtr;
    for (int i = 0; i < TEST_PACKETS_COUNT; i++) {
        XEmacPs_BdSetAddressTx(CurBdPtr, (UINTPTR)&DdrTxFrames[i]);
        XEmacPs_BdSetLength(CurBdPtr, TxFrameLength);
        XEmacPs_BdClearTxUsed(CurBdPtr);
        XEmacPs_BdSetLast(CurBdPtr);
        CurBdPtr = XEmacPs_BdRingNext(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), CurBdPtr);
    }

    /* Submit the configured TX BDs to HW */
    Status = XEmacPs_BdRingToHw(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), TEST_PACKETS_COUNT, BdPtr);
    if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
        Xil_DCacheFlushRange((UINTPTR)BdPtr, 64 * TEST_PACKETS_COUNT);
    }
    ...
    /* ========================================================== */
    /* 3. PHY Initialization and Link Wait                        */
    /* ========================================================== */
    xil_printf("\r\nWaiting for the physical link to come up...\r\n");
    u16 PhyStatus = 0, PhyReg1 = 0, PhyReg2 = 0;
    u32 DetectedPhyAddr = 32;

    for (int i = 0; i <= 31; i++) {
        XEmacPs_PhyRead(EmacPsInstancePtr, i, 2, &PhyReg1);
        XEmacPs_PhyRead(EmacPsInstancePtr, i, 3, &PhyReg2);
        if ((PhyReg1 > 0x0000) && (PhyReg1 < 0xffff) && (PhyReg2 > 0x0000) && (PhyReg2 < 0xffff)) {
            DetectedPhyAddr = i;
            break;
        }
    }

    if (DetectedPhyAddr < 32) {
        do {
            XEmacPs_PhyRead(EmacPsInstancePtr, DetectedPhyAddr, 1, &PhyStatus);
            EmacpsDelay(1);
        } while ((PhyStatus & 0x0004) == 0);
        xil_printf("Link is UP! Starting Continuous MAC Transmission...\r\n");
    } else {
        xil_printf("Error: PHY not found!\r\n");
        return XST_FAILURE;
    }

    /* Disable receiving broadcast packets to block unwanted network noise */
    XEmacPs_ClearOptions(EmacPsInstancePtr, XEMACPS_BROADCAST_OPTION);

    /* ========================================================== */
    /* 4. Start Simultaneous Continuous TX and RX                 */
    /* ========================================================== */

    /* Start the device and initiate the first transmission */
    ...
    /*
     * Infinite loop to continuously reclaim processed descriptors
     * and push them back to the DMA hardware.
     */
    while (1) {

        /* --------------------------------------------------------
         * HANDLE CONTINUOUS RECEPTION (RX)
         * -------------------------------------------------------- */
        int NumRxProcessed = XEmacPs_BdRingFromHwRx(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), RXBD_CNT, &BdRxPtr);

        if (NumRxProcessed > 0) {

            XEmacPs_Bd *CurRxBdPtr = BdRxPtr;

            /* Process, invalidate cache, and print Hex Dump for received packets */
            for (int i = 0; i < NumRxProcessed; i++) {
                int pkt_index = RxFrameIndex % TEST_PACKETS_COUNT;
                TotalPacketsReceived++;

                /* Invalidate cache so the CPU reads fresh data from physical memory (DDR) */
                if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
                    Xil_DCacheInvalidateRange((UINTPTR)&DdrRxFrames[pkt_index], sizeof(EthernetFrame));
                }

//                if ((TotalPacketsReceived % 100) == 0) {
//                    xil_printf("\r\n[LOG] Received %d packets so far...\r\n", TotalPacketsReceived);
//                    xil_printf("--- Hex Dump of Sample Packet [%d] (First 32 Bytes) ---\r\n", pkt_index);
//                    u8 *rx_dump_ptr = (u8 *)&DdrRxFrames[pkt_index];
//                    for (int b = 0; b < 32; b++) {
//                          xil_printf("%02X ", rx_dump_ptr[b]);
//                    }
//                          xil_printf("\r\n----------------------------------------------------\r\n");
//                    }
                RxFrameIndex++;
            ...
            /* Free the processed RX BDs */
            XEmacPs_BdRingFree(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), NumRxProcessed, BdRxPtr);

            /* Re-allocate the RX BDs for incoming traffic */
            XEmacPs_BdRingAlloc(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), NumRxProcessed, &BdRxPtr);

            /* Reassign addresses for future receptions */
            CurRxBdPtr = BdRxPtr;
            for (int i = 0; i < NumRxProcessed; i++) {
                int next_pkt_index = (RxFrameIndex - NumRxProcessed + i) % TEST_PACKETS_COUNT;
                XEmacPs_BdSetAddressRx(CurRxBdPtr, (UINTPTR)&DdrRxFrames[next_pkt_index]);
                CurRxBdPtr = XEmacPs_BdRingNext(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), CurRxBdPtr);
            }

            /* Submit re-allocated BDs to hardware so it continues receiving */
            XEmacPs_BdRingToHw(&(XEmacPs_GetRxRing(EmacPsInstancePtr)), NumRxProcessed, BdRxPtr);
        }

        /* --------------------------------------------------------
         * HANDLE CONTINUOUS TRANSMISSION (TX)
         * -------------------------------------------------------- */
        int NumTxProcessed = XEmacPs_BdRingFromHwTx(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), TXBD_CNT, &BdPtr);

        if (NumTxProcessed > 0) {

            /* Free the processed TX BDs that have successfully been sent */
            XEmacPs_BdRingFree(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), NumTxProcessed, BdPtr);

            /* Allocate new BDs to keep the transmission pipeline full */
            XEmacPs_BdRingAlloc(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), NumTxProcessed, &BdPtr);

            XEmacPs_Bd *CurTxBdPtr = BdPtr;
            for (int i = 0; i < NumTxProcessed; i++) {
                /* Map the newly allocated BD to our 20% duty cycle data in DDR */
                XEmacPs_BdSetAddressTx(CurTxBdPtr, (UINTPTR)&DdrTxFrames[TxFrameIndex % TEST_PACKETS_COUNT]);
                XEmacPs_BdSetLength(CurTxBdPtr, TxFrameLength);
                XEmacPs_BdClearTxUsed(CurTxBdPtr);
                XEmacPs_BdSetLast(CurTxBdPtr);

                CurTxBdPtr = XEmacPs_BdRingNext(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), CurTxBdPtr);
                TxFrameIndex++;
            }

            /* Submit the refreshed BDs to hardware */
            XEmacPs_BdRingToHw(&(XEmacPs_GetTxRing(EmacPsInstancePtr)), NumTxProcessed, BdPtr);

            /* Explicitly kick the DMA to guarantee continuous transmission */
            XEmacPs_Transmit(EmacPsInstancePtr);
        }
    }
...
static void XEmacPsRecvHandler(void *Callback)
{
    ...
    /* Temporarily disable reception-related interrupts */
//    XEmacPs_IntDisable(EmacPsInstancePtr, (XEMACPS_IXR_FRAMERX_MASK |
//        XEMACPS_IXR_RX_ERR_MASK));

    FramesRx++; /* This counter can be scaled according to the number of BDs processed at any given time */

    /* Invalidate the entire receive array to synchronize data with physical memory */
//    if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
//        Xil_DCacheInvalidateRange((UINTPTR)&DdrRxFrames, sizeof(EthernetFrame) * TEST_PACKETS_COUNT);
//    }
//    if (GemVersion > 2) {
//        if (EmacPsInstancePtr->Config.IsCacheCoherent == 0) {
//            Xil_DCacheInvalidateRange((UINTPTR)RxBdSpacePtr, 64 * TEST_PACKETS_COUNT);
//        }
//    }
}
/****************************************************************************/
...
/*****************************************************************************/
/*
 * Generates a step signal with a 20% duty cycle inside the Ethernet payload.
 * 20% of the payload will be HIGH (0xFF) and 80% will be LOW (0x00).
 */
void GenerateDutyCycleSignal(EthernetFrame *FramePtr, u32 PayloadSize) {
    /* Calculate 20% of the total payload size */
    u32 HighBytesCount = (PayloadSize * 20) / 100;

    /* Pointer to the start of the payload data (after the 14-byte MAC header) */
    u8 *Payload = (u8 *)FramePtr + XEMACPS_HDR_SIZE;

    for (u32 i = 0; i < PayloadSize; i++) {
        if (i < HighBytesCount) {
            Payload[i] = 0xFF; /* HIGH state */
        } else {
            Payload[i] = 0x00; /* LOW state */
        }
    }
}
